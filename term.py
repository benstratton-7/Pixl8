...
def make_rand_file_name(infile):
    ...
    timeid = str(time)
    return 'Pixl8ed_' + infile +''.join(filter(str.isdigit, timeid))
...
def main():
    ...
    # Add arguments
    parser.add_argument('--input', default='./example images/matt-damon.jpg', help='Input file path. Type the full path to the file to use images other than the given examples')
    ...
    infilename = os.path.basename(os.path.normpath(input_path))
    ...
    final_path = f'{output_path}/{make_rand_file_name(infilename)}.png'
